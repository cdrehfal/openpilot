  ...
  class EnumProxy(metaclass=EnumProxyMeta):
    def __new__(cls, *args, **kwargs):
      if enum_class.__name__ == 'Ops' and args and isinstance(args[0], int):
        ...
        if value in TG_e837e367_OP_CODE_MAP:
          op_name = TG_e837e367_OP_CODE_MAP[value]
          if op_name in ('CONTIGUOUS', 'RETURNED'):
            op_name = 'NOOP'
          try:
            return getattr(enum_class, op_name)
        ...
        try:
          ...
          print(f"DEBUG: Failed to map integer {value} with enum_offset {enum_offset} and legacy_map {legacy_map}", flush=True)
          raise ValueError("LEGACY_MISMATCH") from None
...
def _dynamic_factory(real_class, enum_offset=0, legacy_map=False):
  ...
  def factory(*args, **kwargs):
    if real_class.__name__ == 'Buffer':
      ...
      # Modern Tinygrad's UOp expects (op, src, arg, tag, metadata), but old pickled models
      # serialized weights as Ops.BUFFER with arg=ParamArg and _buffer=Buffer as a 6th argument.
      if len(args) == 6 and getattr(args[0], 'name', None) == 'BUFFER':
        if type(args[2]).__name__ == 'ParamArg':
          import dataclasses
          new_arg = dataclasses.replace(args[2], buffer=args[5])
          args = (args[0], args[1], new_arg, args[3], args[4])
...
def load_oob(f):
  ...
  try:
    jits = try_load(offset=0, legacy_map=False)
  except ValueError as e:
    if "LEGACY_MISMATCH" in str(e):
      try:
        print("legacy AST detected! Applying legacy Ops mapping...", flush=True)
        jits = try_load(offset=0, legacy_map=True)
      except ValueError as e2:
        if "OFFSET_2" in str(e2):
          jits = try_load(offset=2, legacy_map=True)
        else:
          raise e2
  ...
  # Strip deprecated graph calls from legacy jits because QCOM no longer has a custom python graph runner
  from tinygrad.uop.ops import UOp, Ops
  for v in jits.values():
    if not hasattr(v, 'captured') or not hasattr(v.captured, '_linear'):
      continue
    new_src = []
    for call in v.captured._linear.src:
      if call.op.name == 'CALL' and call.src and call.src[0].op.name == 'CUSTOM_FUNCTION' and getattr(call.src[0], 'arg', None) == 'graph':
        new_src.extend(call.src[0].src[0].src)
      else:
        new_src.append(call)
    v.captured._linear = UOp(Ops.LINEAR, src=tuple(new_src))
    v.captured.linear = v.captured._linear
